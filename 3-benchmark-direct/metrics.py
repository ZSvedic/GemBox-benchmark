...
def print_metrics(metrics: list[Metrics], csv_format: bool = False, run_name: str = '?') -> None:
    ...
    for m in metrics:
        error_rate_str = f"{m.error_rate_avg:.0%}"
        name = f"{m.name},"
        if csv_format:
            id = datetime.now().strftime("%y%m%d%H%M%f")
            api_issue_rate = m.api_issues / m.api_calls if m.api_calls > 0 else 0.0
            print(f"{id}, {run_name}, {m.provider_name}, {name} {m.tokens_mdn}, ${m.cost_mdn:.6f}, {m.time:.2f}, {error_rate_str}, {m.api_issues:.0%}")
        else:
            print(f"\t{name:32.32}\ttokens_mdn={m.tokens_mdn},\tcost_mdn=${m.cost_mdn:.6f},\ttime={m.time:.2f}s,\terror_rate_avg={error_rate_str},\tapi_issues={m.api_issues}/{m.api_calls}")

def main_test():
    print("\n===== metrics.main_test() =====")

    # Create 3 valid metrics:
    v1 = Metrics(name="v1", provider_name="ACME", cost_mdn=0.010, tokens_mdn=100, time=0.20, error_rate_avg=0.20)
    v2 = Metrics(name="v2", provider_name="ACME", cost_mdn=0.015, tokens_mdn=120, time=0.30, error_rate_avg=0.10)
    v3 = Metrics(name="v3", provider_name="ACME", cost_mdn=0.020, tokens_mdn=150, time=0.50, error_rate_avg=0.00)
    ...
    i1 = Metrics(name="i1", provider_name="BAD_ACME", cost_mdn=9.999, tokens_mdn=9999, time=9.99, error_rate_avg=1.00, api_issues=1)
    i2 = Metrics(name="i2", provider_name="BAD_ACME", cost_mdn=8.888, tokens_mdn=8888, time=8.88, error_rate_avg=0.90, api_issues=1)
